...
class OptionalPackages
{
    ...
    public function setPackageName()
    {
        ...
        $this->composerDefinition['name'] = $package_name;
    }

    public function setPackageType()
    {
        $ask[] = "\n  <question>What package type (e.g. library, project) do you want to setup ?</question>\n";
        $ask[] = "  [<comment>1</comment>]library(default)\n";
        $ask[] = "  [<comment>2</comment>]project\n";
        $ask[] = "Make your selection or type a package type, like library (n):\n";
        $answer = $this->io->askAndValidate(
            implode('', $ask),
            function ($value) {
        ...
        $type = in_array($answer, ['n', 'library', '1']) ? 'library' : 'project';
        $this->composerDefinition['type'] = $type;
    }

    public function handleTypeLibrary()
    {
        ...
        $this->walkDir($sourceDir, function ($filename) use ($namespace) {
